...
class Peer(models.Model):
    ...
    number = models.IntegerField(verbose_name='Number', unique=True, help_text="Número", db_column='number')
    ...
    @staticmethod
    def get_peer_by_number(number: int):
        try:
            peer = Peer.objects.get(number=number)
            return peer
        except Peer.DoesNotExist:
            return None
